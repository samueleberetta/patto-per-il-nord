...
import { Search, MapPin, ChevronDown, ChevronUp, Building2, Star, Phone, Mail, ArrowRight } from "lucide-react";
        ...
        <div className="mt-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredFeatured.map((m) => (
              <div
                key={m.id}
                className="group flex flex-col rounded-xl border-2 border-[#1B3A6B]/20 bg-white p-5 transition-all hover:shadow-md hover:border-[#1B3A6B]/50 hover:-translate-y-0.5"
              >
                <div className="flex items-start gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#1B3A6B]/10 group-hover:bg-[#1B3A6B]/20 transition-colors">
                  ...
                  <div className="mt-3 space-y-1.5 border-t pt-3">
                    {m.contact_phone && (
                      <a
                        ...
                        className="flex items-center gap-2 text-sm text-foreground/80 hover:text-[#1B3A6B] transition-colors"
                      >
                        <Phone className="h-3.5 w-3.5 text-muted-foreground" />
                        <span className="truncate">{m.contact_phone}</span>
                      </a>
                    )}
                    {m.contact_email && (
                      <a
                ...
                <Link
                  href={`/comuni/${m.slug}`}
                  className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-[#1B3A6B] hover:underline self-start"
                >
                  Scopri di più
